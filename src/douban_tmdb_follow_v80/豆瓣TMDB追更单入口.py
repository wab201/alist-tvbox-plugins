    ...
    def _legacy_push(self, rows):
        ...
        try:
            if response.status_code < 200 or response.status_code >= 300:
                raise _V80HistoryHttpError(response.status_code, owner._atvp_history_http_error(response, "写入"))
        finally:
            try:
                response.close()
            except Exception:
...
class Spider(BaseSpider):
    ...
    FOLLOW_PREHEAT_ACTION = "tmdb-follow:preheat"
    ...
    ATVP_FAST_SYNC_ACTION = "atvp-follow:fast-sync"
    ...
    FOLLOW_CANDIDATE_FAILURE_COOLDOWN = 30
    ...
    RESOURCE_SEARCH_MODES = ("vod1", "vod", "pansou", "telegram", "plugin")
    ...
    RESOURCE_SEARCH_BUDGET = 8
    RESOURCE_DETAIL_BUDGET = 20
    RESOURCE_FOREGROUND_BUDGET = 10
    RESOURCE_FOREGROUND_DETAIL_SLICE = 2.5
    ...
    def __init__(self):
        ...
        self._cache_health_controller = CacheHealthController(self)
        self._background_bulkhead_controller = BackgroundBulkheadController(
            generation=self._cache_generation,
        )
        self._timeout_budget_controller = TimeoutBudgetController(
            generation=self._cache_generation,
        )
        self._filters = None
        self._filters_at = 0
    ...
    def _v80_timeout_child_scope(self, operation, timeout_seconds, deadline=None):
    ...
    def _init_locked(self, extend=""):
        ...
        with self._cache_persist_lock:
            with self._cache_lock:
                ...
                self._route_quality_dirty = False
                self._route_quality_saving = None
                if self._cache_generation != init_generation:
        ...
        self._cache_health_controller.reset()
        self._filters = None
        self._filters_at = 0
    ...
    def homeVideoContent(self):
        with self._timeout_budget_controller.scope(
            ...
            return self._v80_homeVideoContent_unbounded()

    def _v80_homeVideoContent_unbounded(self):
        self._schedule_entry_resource_preheat()
        try:
    ...
    def categoryContent(self, tid, pg, filter=False, extend=None):
        with self._timeout_budget_controller.scope(
                "category", self.RESOURCE_FOREGROUND_BUDGET,
                expected_generation=self._cache_generation):
            return self._v80_categoryContent_unbounded(tid, pg, filter, extend)

    def _v80_categoryContent_unbounded(self, tid, pg, filter=False, extend=None):
    ...
    def detailContent(self, ids):
        with self._timeout_budget_controller.scope(
                "detail", self.RESOURCE_DETAIL_BUDGET,
                expected_generation=self._cache_generation):
            return self._v80_detailContent_unbounded(ids)

    def _v80_detailContent_unbounded(self, ids):
        subject_id = self._first_id(ids)
        if subject_id.startswith("atvp_detail:"):
            subject_id = subject_id[len("atvp_detail:"):]
        if subject_id.startswith("tmdb:") or subject_id.startswith("local:tv:"):
            detail_deadline = time.monotonic() + self.RESOURCE_FOREGROUND_BUDGET
            followed = self._followed_tmdb_detail(subject_id)
            if followed is not None:
                return self._alist_detail_from_metadata(
                    subject_id, followed, deadline=detail_deadline,
                )
            if subject_id.startswith("tmdb:"):
                return self._alist_detail_from_metadata(
                    subject_id, self._tmdb_detail(subject_id), deadline=detail_deadline,
                )
            return {"list": []}
    ...
    def searchContent(self, key, quick=False, pg="1"):
        with self._timeout_budget_controller.scope(
                "search", self.RESOURCE_SEARCH_BUDGET,
                expected_generation=self._cache_generation):
            return self._v80_searchContent_unbounded(key, quick, pg)

    def _v80_searchContent_unbounded(self, key, quick=False, pg="1"):
        page = self._positive_int(pg, 1)
    ...
    def playerContent(self, flag, id, vipFlags=None):
        with self._timeout_budget_controller.scope(
                "player", self.FOLLOWPLAY_PLAY_BUDGET,
                expected_generation=self._cache_generation):
            return self._v80_playerContent_unbounded(flag, id, vipFlags)

    def _v80_playerContent_unbounded(self, flag, id, vipFlags=None):
            ...
            try:
                ...
                if not output_validated:
                    ...
                    if checked is not None and isinstance(checked.get("output"), dict):
                        output = dict(checked["output"])
                        quality_probe = checked
                    elif self._safe_atvp_play_output(
    ...
    def action(self, action):
        with self._timeout_budget_controller.scope(
                "action", self.RESOURCE_DETAIL_BUDGET,
                expected_generation=self._cache_generation):
            return self._v80_action_unbounded(action)

    def _v80_action_unbounded(self, action):
        value = str(action or "")
        if (
            ...
            or value == self.FOLLOW_PREHEAT_ACTION
        ...
        if value == self.ATVP_FAST_SYNC_ACTION:
            return self._start_atvp_job("quick")
        ...
        try:
            ...
            with self._v80_timeout_child_scope("douban_wish", self.timeout) as operation:
    ...
    def _follow_candidate_card(self, candidate, mode="view"):
        ...
        if (
            ...
            if state == "running":
                ...
                message = str(status.get("message") or remark)
                remark = message + " · 点击操作状态卡清除后可重试"
                action = self.ERROR_PREFIX + quote(message, safe="")
    ...
    def _start_follow_candidate_add(self, payload):
        ...
        failed_at = self._positive_int(last.get("updated_at"), 0)
        elapsed = max(0, int(time.time()) - failed_at) if failed_at else self.FOLLOW_CANDIDATE_FAILURE_COOLDOWN
        if (
                str(last.get("state") or "") == "failed"
                and str(last.get("operation") or "") == "candidate"
                and str(last.get("title") or "") == title
                and elapsed < self.FOLLOW_CANDIDATE_FAILURE_COOLDOWN):
            remaining = self.FOLLOW_CANDIDATE_FAILURE_COOLDOWN - elapsed
            return json.dumps({
                "msg": "已停止自动重试：%s；请%d秒后手动重试" % (
                    str(last.get("message") or "上次确认追更失败"), remaining,
                ),
            }, ensure_ascii=False)
            ...
            try:
                ...
                if not isinstance(item, dict):
                    labels = {
                        ...
                        "movie_conflict": "匹配结果为电影或单集视频",
                        "single_episode": "该项目为单集内容，不支持追更",
                    }
                    raise RuntimeError(labels.get(str(reason or ""), str(reason or "无法确认剧集")))
                key = self._follow_item_key(item)
                if not key:
                    raise RuntimeError("剧集身份无效")
                with self._history_context_lock:
    ...
    def _request_follow_candidate_clear(self, payload):
        ...
        return self._execute_follow_candidate_clear(pending["nonce"])
    ...
    def _category_follow_updates(self, page):
        ...
        self._reconcile_follow_histories(histories)
        followed = list((self._follow_memory.get("items") or {}).values())
        paired = [(item, self._atvp_history_for_item(item, histories)) for item in followed]
        paired.sort(key=self._follow_update_sort_key)
        start = (page - 1) * self.follow_page_size
        selected_pairs = paired[start:start + self.follow_page_size]
        selected = [item for item, _history in selected_pairs]
        self._refresh_follow_page_async(selected)
        state_items = self._follow_memory.get("items") or {}
        refreshed = [state_items.get(self._follow_item_key(item), item) for item in selected]
    ...
    def _follow_update_sort_key(self, pair):
        item, history = pair
        conflict = bool(isinstance(history, dict) and history.get("_follow_history_conflict"))
        if conflict:
            group = 0
        elif str(item.get("check_error") or ""):
            group = 5
        elif self._has_follow_update(item, history):
            group = 1
        elif self._history_can_resume(history or item):
            group = 2
        elif str(item.get("next_air_date") or ""):
            group = 3
        else:
            group = 4
        return (
            group,
            str(item.get("next_air_date") or "9999-99-99"),
            str(item.get("title") or ""),
            self._positive_int(item.get("tmdb_id"), 0),
        )
    ...
    def _refresh_follow_page_async(self, items):
        ...
        due = [
            ...
            if self._follow_item_key(item)
            and (
                now - self._positive_int(item.get("last_checked"), 0) >= self.follow_check_ttl
                or (
                    self._positive_int(item.get("tmdb_id"), 0)
                    and not self._follow_title_alias_values(item, include_primary=False)
        ...
        with self._cache_lock:
            generation = self._cache_generation
        job_key = "refresh:%s:%s" % (
            generation, ",".join(sorted(self._follow_item_key(item) for item in due)),
        )
        job_owner = object()
        with self._follow_enrich_lock:
            if job_key in self._follow_enrich_jobs:
            ...
            try:
                ...
                try:
                    for future in as_completed(futures):
                        ...
                        old_key = self._follow_item_key(source)
                        try:
                            refreshed = future.result()
                            updates[old_key] = refreshed if isinstance(refreshed, dict) else dict(source)
                        except Exception as exc:
                            failed = dict(source)
                            failed["check_error"] = self._short_error(exc)
                            failed["last_checked"] = int(time.time())
                            updates[old_key] = failed
                finally:
                    for future in futures:
                        if not future.done():
                            future.cancel()
                        ...
                        with self._follow_enrich_lock:
                            ...
                            for old_key, item in updates.items():
                                if old_key not in current:
                                    continue
                                previous = current.pop(old_key)
                                new_key = self._follow_item_key(item) or old_key
                                merged = self._merge_follow_identity_items(previous, item)
                                if new_key in current:
                                    merged = self._merge_follow_identity_items(current[new_key], merged)
                                current[new_key] = merged
                            self._save_follow_state(current)
                    self._refresh_follow_categories("follow-page")
            finally:
                with self._follow_enrich_lock:
                    if self._follow_enrich_jobs.get(job_key) is job_owner:
                        self._follow_enrich_jobs.pop(job_key, None)
    ...
    def _toggle_history_share_policy(self, kind):
        ...
        if kind not in ("follow", "watch"):
            return json.dumps({"msg": "播放记录同步设置无效"}, ensure_ascii=False)
        with self._history_context_lock:
            current = dict(self._history_share_policy or {"follow": True, "watch": True})
            ...
            if not self._persist_history_share_policy(candidate):
                return json.dumps({"msg": "播放记录同步设置未能保存，本机状态未改变"}, ensure_ascii=False)
    ...
    def _history_management_cards(self):
        ...
        fast_sync_card = {
            "vod_id": self.ATVP_FAST_SYNC_ACTION,
            "vod_name": "快速同步播放记录",
            "vod_pic": "",
            "vod_remarks": "增量通道 · %s" % (
                self._atvp_status_remark("quick") or "只拉取近期播放进度，不上传本机记录",
            ),
            "action": self.ATVP_FAST_SYNC_ACTION,
        }
        policy = dict(self._history_share_policy or {"follow": True, "watch": True})
        policy_loaded = bool(self._history_share_policy_loaded)
        cards = [sync_card, fast_sync_card]
        for kind, label in (("follow", "追更播放记录"), ("watch", "观看记录")):
            ...
            cards.append({
                ...
                "vod_name": "%s同步：%s" % (
                    label,
                    ("已开启" if enabled else "已关闭") if policy_loaded else "等待确认",
                ),
                "vod_pic": "",
                "vod_remarks": (
                    "本机设置暂未读取，已暂停上传；点击确认同步设置"
                    if not policy_loaded else
                    ("点击关闭" if enabled else "点击开启")
                    + "；不影响云端读取和已有记录"
                ),
    ...
    def _follow_preheat_card(self):
        progress = self._follow_preheat_snapshot()
        total = progress["total"]
        ready = progress["ready"]
        processed = progress["processed"]
        if progress["running"]:
            remark = "预热中 · 已处理 %d/%d · 可直接播放 %d/%d" % (
                processed, total, ready, total,
            )
            if progress["route_jobs"]:
                remark += " · 线路验证 %d" % progress["route_jobs"]
        elif total and processed >= total and progress["started_at"]:
            remark = "预热完成 · 可直接播放 %d/%d" % (ready, total)
            if ready < total:
                remark += " · %d 项未找到可用线路" % (total - ready)
            else:
                remark += " · 线路失效后自动刷新"
        elif total:
        ...
        return {
            ...
            "vod_pic": "",
            "vod_remarks": remark,
            "action": self.FOLLOW_PREHEAT_ACTION,
        }

    def _start_follow_preheat(self):
        items = [
            dict(item) for item in (self._follow_memory.get("items") or {}).values()
            if isinstance(item, dict) and str(item.get("title") or "").strip()
        ]
        if not items:
            result = json.dumps({"msg": "当前没有可预热的追更剧集"}, ensure_ascii=False)
            return self._remember_follow_action_result(result, "preheat")
        if not (
                self._alist_tvbox_plugin and self.route_preheat and self.atvp_api
                and self.atvp_token and self._atvp_session is not None):
            result = json.dumps({"msg": "主动预热未启动：AList-TVBox播放通道尚未就绪"}, ensure_ascii=False)
            return self._remember_follow_action_result(result, "preheat")
        progress = self._start_follow_preheat_all(items)
        card = self._follow_preheat_card()
        if progress.get("accepted"):
            message = "主动预热已开始 · " + str(card.get("vod_remarks") or "后台处理中")
            self._set_follow_action_status("running", message, "preheat")
            self._refresh_follow_categories()
            return json.dumps({"msg": message}, ensure_ascii=False)
        message = "主动预热正在进行 · " + str(card.get("vod_remarks") or "请稍后查看")
        result = json.dumps({"msg": message}, ensure_ascii=False)
        return self._remember_follow_action_result(result, "preheat")

    def _history_alert_cards(self):
        cards = []
        for kind, action, running_label, failed_label in (
                ("sync", self.ATVP_SYNC_ACTION, "播放记录正在同步", "播放记录同步失败"),
                ("quick", self.ATVP_FAST_SYNC_ACTION, "快速同步正在进行", "快速同步失败"),
        ):
            ...
            cards.append({
                "vod_id": action,
                "vod_name": running_label if state == "running" else failed_label,
                "vod_pic": "",
    ...
    def _request_follow_confirmation(self, operation, raw_id):
        ...
        item_key, item = self._follow_item_lookup(raw_id)
        if operation not in ("seen", "remove") or not item_key:
        ...
        raw_nonce = "%s:%s:%s:%s" % (operation, item_key, repr(time.time()), threading.get_ident())
        pending = {
            "nonce": hashlib.sha256(raw_nonce.encode("utf-8")).hexdigest()[:16],
            "operation": operation,
            "item_key": item_key,
        ...
        self._persist_follow_action_state()
        self._refresh_follow_categories()
        label = "标记当前更新已看完" if operation == "seen" else "取消追更"
    ...
    def _execute_follow_confirmation(self, payload):
        ...
        item_key, _item = self._follow_item_lookup(raw_id)
        with self._follow_action_state_lock:
            pending = dict((self._follow_action_state or {}).get("pending") or {})
        requested_at = self._positive_int(pending.get("requested_at"), 0)
        pending_key = str(pending.get("item_key") or pending.get("tmdb_id") or "").strip()
        same_pending = bool(nonce and nonce == str(pending.get("nonce") or ""))
        valid = (
            same_pending
            and operation == str(pending.get("operation") or "")
            and item_key == pending_key
        ...
        result = self._follow_action(operation, item_key)
    ...
    def _follow_item_key(self, item):
        if not isinstance(item, dict):
            return ""
        tmdb_id = self._positive_int(item.get("tmdb_id"), 0)
        if tmdb_id:
            return str(tmdb_id)
        source_id = str(item.get("source_id") or item.get("local_source_id") or "").strip()
        if re.match(r"^local:tv:[0-9a-f]{24}$", source_id):
            return source_id
        douban_id = self._subject_id(item.get("douban_id"))
        return "douban:" + douban_id if douban_id else ""

    def _follow_item_source_aliases(self, item):
        aliases = set()
        if not isinstance(item, dict):
            return aliases
        for value in (item.get("source_id"), item.get("local_source_id")):
            value = str(value or "").strip()
            if value:
                aliases.add(value)
        for value in item.get("source_aliases") or []:
            value = str(value or "").strip()
            if value:
                aliases.add(value)
        tmdb_id = self._positive_int(item.get("tmdb_id"), 0)
        if tmdb_id:
            aliases.add("tmdb:tv:%s" % tmdb_id)
        return aliases

    def _follow_item_lookup(self, raw_id):
        raw = unquote(str(raw_id or "").strip())
        items = self._follow_memory.get("items") if isinstance(self._follow_memory, dict) else {}
        if not isinstance(items, dict):
            return "", None
        numeric = self._positive_int(raw, 0)
        direct_key = str(numeric) if numeric else raw
        direct = items.get(direct_key)
        if isinstance(direct, dict):
            return direct_key, direct
        for key, item in items.items():
            if isinstance(item, dict) and raw in self._follow_item_source_aliases(item):
                return str(key), item
        return "", None

    def _follow_progress_rank(self, item):
        if not isinstance(item, dict):
            return (0, 0, 0, 0)
        return (
            self._positive_int(item.get("history_source_updated_at_ms"), 0),
            self._positive_int(item.get("history_updated_at"), 0),
            self._episode_rank(item.get("history_episode") or item.get("tracked_episode")),
            self._positive_int(item.get("history_position"), 0),
        )

    def _merge_follow_identity_items(self, current, incoming):
        current = dict(current or {})
        incoming = dict(incoming or {})
        output = dict(current)
        output.update(incoming)
        progress = incoming if self._follow_progress_rank(incoming) >= self._follow_progress_rank(current) else current
        for key in (
                "history_episode", "history_position", "history_duration", "history_vod_name",
                "history_completed", "history_updated_at", "history_source_updated_at_ms",
                "history_playback", "tracked_episode", "seen_episode", "seen_source",
                "last_play_route", "alist_vod_id", "alist_resource_mode",
                "alist_resource_provider", "binding_updated_at"):
            if key in progress:
                output[key] = progress[key]
        aliases = self._follow_item_source_aliases(current) | self._follow_item_source_aliases(incoming)
        if aliases:
            output["source_aliases"] = sorted(aliases)
        return output
    ...
    def _follow_state_cards(self, include_pending=False):
        ...
        if include_pending and pending and requested_at and int(time.time()) - requested_at <= self.FOLLOW_CONFIRM_TTL:
            operation = str(pending.get("operation") or "")
            item_key = str(pending.get("item_key") or pending.get("tmdb_id") or "").strip()
            nonce = str(pending.get("nonce") or "")
            title = str(pending.get("title") or item_key)
            if operation in ("seen", "remove") and item_key and nonce:
                label = "标记当前更新已看完" if operation == "seen" else "取消追更"
                execute = self.FOLLOW_EXECUTE_PREFIX + "%s:%s:%s" % (nonce, operation, item_key)
                cards.extend([
                    {
                        "vod_id": execute,
    ...
    def _follow_action(self, operation, raw_id, title=""):
        operation = str(operation or "").strip().lower()
        item_key, existing_item = self._follow_item_lookup(raw_id)
        tmdb_id = self._positive_int(raw_id, 0)
        try:
            if operation == "remove":
                if not item_key or not isinstance(existing_item, dict):
                ...
                with self._follow_enrich_lock:
                    ...
                    if not isinstance(current, dict):
                        return json.dumps({"msg": "该剧集尚未追更"}, ensure_ascii=False)
                    title = str(current.get("title") or item_key)
            ...
            if operation == "seen":
                if not item_key or not isinstance(existing_item, dict):
                    return json.dumps({"msg": "该剧集尚未追更"}, ensure_ascii=False)
                tmdb_id = self._positive_int(existing_item.get("tmdb_id"), 0)
                data = None
                if tmdb_id:
                    self._require_tmdb_credentials()
                ...
                with self._follow_enrich_lock:
                    items = dict(self._follow_memory.get("items") or {})
                    current = dict(items.get(item_key) or {})
                    if not current:
                        return json.dumps({"msg": "该剧集已取消追更"}, ensure_ascii=False)
                    item = self._follow_item_from_tmdb(data, current) if isinstance(data, dict) else current
                    episode = str(
                        item.get("latest_episode") or item.get("history_episode")
                        or item.get("tracked_episode") or ""
                    )
                    if not episode:
                        return json.dumps({"msg": "当前没有可标记的分集进度"}, ensure_ascii=False)
                    item["seen_episode"] = episode
                    item["tracked_episode"] = episode
                    item["seen_source"] = "manual"
                    items[item_key] = item
                    self._save_follow_state(items)
                return json.dumps({"msg": "已标记看到 " + episode}, ensure_ascii=False)
            if not tmdb_id:
    ...
    def _refresh_follow_categories(self, source=""):
        ...
        last_native = getattr(self, "_follow_native_category_refresh_at", 0.0)
        if source in ("follow-page", "history-snapshot") and last_native and (
    ...
    def _sanitize_history_playback(self, value):
        if not isinstance(value, dict):
            return {}
        output = {}
        text_limits = {
        ...
        for key, limit in text_limits.items():
            text = _history_clip_text(value.get(key), limit).strip()
            if not text or self._contains_url_reference(text):
                continue
            output[key] = text
        if output.get("sourceKind") not in ("site", "spider_plugin"):
            output.pop("sourceKind", None)
        if output.get("resourceMode") not in self.RESOURCE_SEARCH_MODES:
            output.pop("resourceMode", None)
        provider = self._resource_provider_key(output.get("resourceProvider"))
        if provider:
            output["resourceProvider"] = provider
        else:
            output.pop("resourceProvider", None)
        integer_keys = (
            "playlistIndex", "sourceGroupIndex", "sourceIndex", "sourceSubgroupIndex",
        )
        for key in integer_keys:
            if key in value and not isinstance(value.get(key), bool):
                output[key] = self._bounded_int(value.get(key), 0, 0, 2147483647)
        if isinstance(value.get("completed"), bool):
            output["completed"] = value["completed"]
        updated_at = self._bounded_int(value.get("updatedAt"), 0, 0, 9223372036854775807)
        if updated_at:
    ...
    def _sanitize_follow_persisted_item(self, item):
        output = dict(item or {})
        source_id = _history_clip_text(output.get("source_id"), 2048).strip()
        local_source_id = _history_clip_text(output.get("local_source_id"), 2048).strip()
        aliases = []
        for value in list(output.get("source_aliases") or []) + [source_id, local_source_id]:
            value = _history_clip_text(value, 2048).strip()
            if value and not self._contains_url_reference(value) and value not in aliases:
                aliases.append(value)
        if aliases:
            output["source_aliases"] = aliases[:8]
        else:
            output.pop("source_aliases", None)
        if local_source_id and re.match(r"^local:tv:[0-9a-f]{24}$", local_source_id):
            output["local_source_id"] = local_source_id
        else:
            output.pop("local_source_id", None)
        if not self._positive_int(output.get("tmdb_id"), 0) and re.match(r"^local:tv:[0-9a-f]{24}$", source_id):
            output["source_id"] = source_id
            output["media_type"] = "tv"
            output["pending_metadata"] = True
            output["metadata_source"] = "local_series"
        ...
        playback = self._sanitize_history_playback(output.get("history_playback"))
        if playback:
            output["history_playback"] = playback
        else:
            output.pop("history_playback", None)
    ...
    def _follow_card(self, item, action_mode="", history=None):
        tmdb_id = self._positive_int(item.get("tmdb_id"), 0)
        item_key = self._follow_item_key(item)
        title = str(item.get("title") or (("TMDB剧集 %s" % tmdb_id) if tmdb_id else "本地剧集"))
        ...
        if action_mode == "seen" and item_key:
            ...
            card["vod_remarks"] = "待确认标记当前更新已看完 · " + card["vod_remarks"]
        elif action_mode == "remove" and item_key:
            card["action"] = self.FOLLOW_REMOVE_PREFIX + item_key
            card["vod_remarks"] = "待确认取消追更 · " + card["vod_remarks"]
        return card

    def _follow_remark(self, item, history=None):
        if isinstance(history, dict) and history.get("_follow_history_conflict"):
            labels = {
                "strong_identity": "TMDB或来源编号冲突",
                "playback_identity": "播放器来源身份冲突",
                "resource_identity": "网盘资源身份冲突",
                "drive_identity": "网盘路径身份冲突",
                "ambiguous_title": "标题匹配不唯一",
            }
            reason = labels.get(
                str(history.get("_follow_history_conflict_reason") or ""),
                "内容身份冲突",
            )
            return "播放记录待确认（%s） · 不自动更新追更进度" % reason
        ...
        if isinstance(history, dict):
            state.update(self._history_resume_fields(item, history))
        latest = str(state.get("latest_episode") or "")
        seen = str(state.get("seen_episode") or "")
        current = str(state.get("history_episode") or "")
        completed = state.get("history_completed")
        if not isinstance(completed, bool) and current:
            completed = self._history_is_complete({
                "position": state.get("history_position"),
                "duration": state.get("history_duration"),
            })
        parts = []
        if latest:
            parts.append("更新至 " + latest)
        if seen:
            parts.append("已看 " + seen)
        action = ""
        if current and completed is True:
            next_episode = self._next_episode_after(current, latest)
            if next_episode:
                parts.append("下一集 " + next_episode)
                action = "播放下一集 " + next_episode
        elif current:
            position = self._positive_int(state.get("history_position"), 0)
            progress = self._format_millis(position)
            parts.append("正在看 " + current + ((" " + progress) if progress else ""))
            action = "继续观看 " + current
        error = str(state.get("check_error") or "").strip()
        if error:
            parts.append("检查失败：" + error)
        if not parts:
            parts.append(str(state.get("status") or "已追更"))
        if action:
            parts.append(action)
        return " · ".join(parts)

    @staticmethod
    def _next_episode_after(current, latest):
        current_match = re.match(r"^S(\d{2})E(\d{2,3})$", str(current or ""))
        latest_match = re.match(r"^S(\d{2})E(\d{2,3})$", str(latest or ""))
        if not current_match or not latest_match:
            return ""
        current_season, current_episode = map(int, current_match.groups())
        latest_season, latest_episode = map(int, latest_match.groups())
        if (latest_season, latest_episode) <= (current_season, current_episode):
            return ""
        if latest_season == current_season:
            return "S%02dE%02d" % (current_season, current_episode + 1)
        return "S%02dE01" % (current_season + 1)
    ...
    def _schedule_atvp_history_refresh(self, cache_key, lightweight=False, status_kind=""):
        ...
        if not lightweight:
            self._set_atvp_status(
                "sync", "running", "播放记录后台同步中 · %s" % self._history_sync_mode_label(),
                persist=False,
            )
            ...
            try:
                ...
                if lightweight:
                    ...
                    if status_kind:
                        self._set_atvp_status(
                            status_kind, "done",
                            "快速同步完成 · %d 条播放记录，%d 条追更进度更新" % (
                                len(histories), progress,
                            ),
                        )
                    self._refresh_follow_categories("history-snapshot")
                else:
                    with self._history_sync_lock:
                    ...
                    if self._history_generation_active(generation):
                        ...
                        if lightweight:
                            ...
                            if status_kind:
                                self._set_atvp_status(
                                    status_kind, "failed",
                                    "快速同步失败：%s" % self._short_error(exc),
                                )
                        else:
                            self._set_atvp_status(
                                "sync", "failed", "播放记录后台同步失败：%s" % self._short_error(exc),
                            )
                        self._refresh_follow_categories(
                            "history-snapshot" if lightweight else "history-sync"
                        )
            finally:
                with self._atvp_job_lock:
            ...
            with self._history_context_lock:
                with self._atvp_job_lock:
                    if self._atvp_job_owners.get(job_kind) is job_owner:
                        ...
                        if not lightweight:
                            self._set_atvp_status(
                                "sync", "failed", "播放记录后台同步启动失败：%s" % self._short_error(exc),
                                persist=False,
                            )
    ...
    def _sync_history_once(self, expected_generation=None):
        ...
        local_rows = v80_history_refresh_local_rows(self, local_rows)
    ...
    def _history_sync_message(result):
        ...
        updated = max(0, int(result.get("uploaded") or 0)) + max(0, int(result.get("imported") or 0))
        summary = "%s 条播放记录更新 · %s 条追更进度更新" % (
            updated, max(0, int(result.get("progress") or 0)),
        )
        if errors:
            failures = "；".join("%s：%s" % (stage, message) for stage, message in errors)
            return "同步部分完成 · %s；失败：%s；%s；模式 %s" % (
                summary, failures, detail, result.get("mode") or "只读",
            )
        return "同步完成 · %s；%s；模式 %s" % (
            summary, detail, result.get("mode") or "只读",
        )
    ...
    def _atvp_probe_card(self, status_remark=""):
        ...
        return {
            ...
            "vod_name": "检查同步连接",
            "vod_pic": "",
            "vod_remarks": status_remark or ("点击检查客户端到 AList-TVBox 的同步连接" if ready else "点击自动识别地址并检查同步连接"),
            "action": self.ATVP_PROBE_ACTION,
        }

    def _start_atvp_job(self, kind):
        if kind not in ("probe", "sync", "quick"):
        ...
        if kind == "quick":
            with self._history_context_lock:
                with self._atvp_job_lock:
                    if "sync" in self._atvp_jobs or "sync-background" in self._atvp_jobs:
                        return json.dumps({"msg": "完整播放记录同步正在进行，请稍后再执行快速同步"}, ensure_ascii=False)
                    if "snapshot-background" in self._atvp_jobs:
                        return json.dumps({"msg": "快速播放进度同步正在进行，请稍后查看卡片状态"}, ensure_ascii=False)
                    self._set_atvp_status(
                        "quick", "running", "快速播放进度同步已开始，请稍后查看卡片结果", persist=False,
                    )
            self._refresh_follow_categories("atvp-job-start")
            submitted = self._schedule_atvp_history_refresh(
                "atvp-history-snapshot", lightweight=True, status_kind="quick",
            )
            if not submitted:
                self._set_atvp_status("quick", "failed", "快速播放进度同步后台任务繁忙，请稍后重试")
                self._refresh_follow_categories("atvp-job-start")
                return json.dumps({"msg": "快速播放进度同步后台任务繁忙，请稍后重试"}, ensure_ascii=False)
            return json.dumps({"msg": "快速播放进度同步已开始，完成后本页会刷新"}, ensure_ascii=False)
        job_owner = object()
        with self._history_context_lock:
            with self._atvp_job_lock:
                if kind == "sync" and "sync-background" in self._atvp_jobs:
                    return json.dumps({"msg": "播放记录后台同步正在进行，请稍后查看管理页状态"}, ensure_ascii=False)
        ...
        # FongMi 不一定会把 action 返回正文渲染成卡片；先发布 running 状态，
        # 下一次分类读取即可看到即时反馈。
        self._refresh_follow_categories("atvp-job-start")
        def worker():
    ...
    def _run_atvp_job(self, kind, generation=None, job_owner=None):
        try:
            with self._history_context_lock:
                self._require_history_generation(generation)
                self._persist_atvp_status()
    ...
    def _v80_atvp_history_request_unbounded(
        ...
        request_kwargs = {"verify": self.verify_tls}
    ...
    def _atvp_history_delete_legacy(self, key):
        ...
        for origin in origins:
            try:
                ...
                try:
                    if response.status_code < 200 or response.status_code >= 300:
                        raise _V80HistoryHttpError(response.status_code, self._atvp_history_http_error(response, "删除"))
                finally:
                    try:
                        response.close()
                    except Exception:
    ...
    def _history_merge_rank(self, row):
        return self._history_semantic_rank(row) + (
            1 if self._history_has_followplay_reference(row) else 0,
        )
    ...
    def _history_playback_state(self, history):
        if not isinstance(history, dict):
            return {}
        direct = self._sanitize_history_playback(
            history.get("history_playback") if isinstance(history.get("history_playback"), dict) else history
        )
        identity = self._history_identity(history)
        if identity:
            source_key, vod_id = identity
            source_kind = direct.get("sourceKind") or dict(
                getattr(self, "_v80_history_source_kinds", {}) or {}
            ).get((source_key, vod_id), "")
            states = getattr(self, "_v80_history_playback_states", {})
            mapped = states.get((source_kind, source_key, vod_id)) if source_kind else None
            if mapped is None and isinstance(states, dict):
                matches = [
                    value for key, value in states.items()
                    if isinstance(key, tuple) and len(key) == 3 and key[1:] == (source_key, vod_id)
                ]
                mapped = matches[0] if len(matches) == 1 else None
            row_updated_at = self._positive_int(
                history.get("updatedAt") or history.get("createTime"), 0,
            )
            if (
                    isinstance(mapped, dict)
                    and row_updated_at
                    and self._positive_int(mapped.get("updatedAt"), 0) < row_updated_at):
                mapped = None
            merged = dict(mapped or {})
            merged.update(direct)
            merged.setdefault("sourceKind", source_kind)
            merged.setdefault("sourceKey", source_key)
            merged.setdefault("vodId", vod_id)
            direct = self._sanitize_history_playback(merged)
        ...
        for source, target in (
            ...
            if payload.get(source) and not direct.get(target):
                direct[target] = payload[source]
        return self._sanitize_history_playback(direct)

    def _history_with_playback_state(self, history):
        if not isinstance(history, dict):
            return history
        state = self._history_playback_state(history)
        if not state:
            return history
        output = dict(history)
        output["history_playback"] = state
        for key in (
            "sourceKind", "sourceKey", "vodId", "completed", "updatedAt",
            "clientKey", "playlistIndex", "sourceGroupIndex", "sourceIndex",
            "sourceSubgroupIndex", "sourceSubgroupName", "driveDirId",
            "driveShareKey", "drivePath",
        ):
            if key in state:
    ...
    @staticmethod
    def _history_conflict(reason):
        return {"_follow_history_conflict": True, "_follow_history_conflict_reason": str(reason or "identity")}

    @staticmethod
    def _history_playback_identity(state):
        if not isinstance(state, dict):
            return None
        values = tuple(str(state.get(key) or "").strip() for key in ("sourceKind", "sourceKey", "vodId"))
        return values if all(values) else None

    def _history_resource_identity(self, state):
        if not isinstance(state, dict):
            return None
        provider = self._resource_provider_key(state.get("resourceProvider"))
        mode = str(state.get("resourceMode") or "").strip().lower()
        resource_id = str(state.get("resourceId") or "").strip()
        return (provider, mode, resource_id) if provider and mode and resource_id else None

    @staticmethod
    def _history_drive_identity(state):
        if not isinstance(state, dict):
            return None
        share_key = str(state.get("driveShareKey") or "").strip()
        path = unicodedata.normalize("NFKC", str(state.get("drivePath") or ""))
        path = re.sub(r"/+", "/", path.replace("\\", "/")).rstrip("/").casefold()
        return (share_key, path) if share_key and path else None
    ...
    def _atvp_history_for_item(self, item, histories):
        if not isinstance(item, dict) or not isinstance(histories, list):
            return None
        tmdb_value = self._positive_int(item.get("tmdb_id"), 0)
        tmdb_id = str(tmdb_value) if tmdb_value else ""
        source_ids = self._follow_item_source_aliases(item)
        douban_id = str(item.get("douban_id") or "").strip()
        if douban_id:
            source_ids.add(douban_id)
        exact = []
        conflicts = []
        for history in histories:
            ...
            if not payload:
                continue
            payload_tmdb_value = self._positive_int(payload.get("tmdbId"), 0)
            payload_tmdb = str(payload_tmdb_value) if payload_tmdb_value else ""
            payload_source = str(payload.get("sourceId") or "").strip()
            comparisons = []
            if tmdb_id and payload_tmdb:
                comparisons.append(payload_tmdb == tmdb_id)
            if source_ids and payload_source:
                comparisons.append(payload_source in source_ids)
            if comparisons and any(comparisons) and not all(comparisons):
                conflicts.append(history)
            elif comparisons and all(comparisons):
                exact.append(self._history_with_playback_state(history))
        if exact:
            return max(exact, key=self._history_semantic_rank)
        if conflicts:
            return self._history_conflict("strong_identity")

        binding_keys = [tmdb_id] + sorted(source_ids)
        bound_id = next((
            str(self.follow_alist_bindings.get(key) or "").strip()
            for key in binding_keys if key and str(self.follow_alist_bindings.get(key) or "").strip()
        ), str(item.get("alist_vod_id") or "").strip())
        if bound_id:
            candidates = [
                self._history_with_playback_state(history) for history in histories
                if self._history_identity(history) and self._history_identity(history)[1] == bound_id
            ]
            expected_identity = self._history_playback_identity(item.get("history_playback"))
            expected_resource = self._history_resource_identity({
                "resourceProvider": item.get("alist_resource_provider"),
                "resourceMode": item.get("alist_resource_mode"),
                "resourceId": item.get("alist_vod_id"),
            })
            expected_drive = self._history_drive_identity(item.get("history_playback"))
            if expected_identity:
                matched = [
                    history for history in candidates
                    if self._history_playback_identity(self._history_playback_state(history)) == expected_identity
                ]
                if candidates and not matched:
                    return self._history_conflict("playback_identity")
            elif expected_resource:
                matched = [
                    history for history in candidates
                    if self._history_resource_identity(self._history_playback_state(history)) == expected_resource
                ]
                explicit = [
                    history for history in candidates
                    if self._history_resource_identity(self._history_playback_state(history)) is not None
                ]
                if explicit and not matched:
                    return self._history_conflict("resource_identity")
            else:
                identities = {
                    self._history_playback_identity(self._history_playback_state(history))
                    for history in candidates
                } - {None}
                if len(identities) > 1:
                    return self._history_conflict("playback_identity")
                matched = candidates
            if expected_drive and matched:
                drive_matches = [
                    history for history in matched
                    if self._history_drive_identity(self._history_playback_state(history)) == expected_drive
                ]
                explicit_drive = [
                    history for history in matched
                    if self._history_drive_identity(self._history_playback_state(history)) is not None
                ]
                if explicit_drive and not drive_matches:
                    return self._history_conflict("drive_identity")
                if drive_matches:
                    matched = drive_matches
            if matched:
                return max(matched, key=self._history_semantic_rank)

        item_resource = self._history_resource_identity({
            "resourceProvider": item.get("alist_resource_provider"),
            "resourceMode": item.get("alist_resource_mode"),
            "resourceId": item.get("alist_vod_id"),
        })
        if item_resource:
            ...
            for history in histories:
                state = self._history_playback_state(history)
                resource = self._history_resource_identity(state)
                if resource == item_resource:
                    matched.append(self._history_with_playback_state(history))
                elif resource and resource[2] == item_resource[2]:
        ...
        item_drive = self._history_drive_identity(item.get("history_playback"))
        if item_drive:
            matched = [
                self._history_with_playback_state(history) for history in histories
                if self._history_drive_identity(self._history_playback_state(history)) == item_drive
            ]
            if matched:
                return max(matched, key=self._history_semantic_rank)
        ...
        return self._history_with_playback_state(best[0]) if len(best) == 1 else self._history_conflict("ambiguous_title")
    ...
    def _history_resume_fields(self, item, history):
        ...
        if not episode:
            return {}
        payload = self._history_followplay_payload(history) or {}
        playback = self._history_playback_state(history)
        ...
        completed = playback.get("completed")
        if isinstance(completed, bool):
            fields["history_completed"] = completed
        source_updated_at = self._bounded_int(
            playback.get("updatedAt") or history.get("updatedAt") or history.get("createTime"),
            0, 0, 9223372036854775807,
        )
        if source_updated_at:
            fields["history_source_updated_at_ms"] = source_updated_at
        if playback:
            fields["history_playback"] = playback
        if resource_id:
    ...
    def _history_rewatch_allowed(self, item, resume_fields):
        current = self._history_playback_identity(item.get("history_playback"))
        candidate = self._history_playback_identity(resume_fields.get("history_playback"))
        if not current or current != candidate:
            return False
        current_time = self._positive_int(item.get("history_source_updated_at_ms"), 0)
        candidate_time = self._positive_int(resume_fields.get("history_source_updated_at_ms"), 0)
        return candidate_time > current_time

    def _reconcile_follow_tombstones(self, tombstones):
        values = [value for value in tombstones or [] if isinstance(value, dict)]
        if not values:
            return 0
        with self._follow_enrich_lock:
            items = dict(self._follow_memory.get("items") or {})
            ...
            clear_keys = (
                "history_episode", "history_position", "history_duration", "history_vod_name",
                "history_completed", "history_updated_at", "history_source_updated_at_ms",
                "history_playback", "last_play_route",
            )
            for key, value in list(items.items()):
                if not isinstance(value, dict):
                    continue
                playback = self._sanitize_history_playback(value.get("history_playback"))
                if not playback or not any(_v80_history_tombstone_matches(row, playback) for row in values):
                    continue
                item = dict(value)
    ...
    def _reconcile_follow_histories(self, histories, changed_items=None):
        tombstones = list(getattr(self, "_v80_history_tombstones", []) or [])
        self._v80_history_tombstones = []
        tombstone_changed = self._reconcile_follow_tombstones(tombstones)
        if not isinstance(histories, list) or not histories:
            return tombstone_changed
        if tombstones:
            histories = [
                history for history in histories
                if not any(
                    _v80_history_tombstone_matches(tombstone, _v80_history_event(history, self))
        ...
        with self._follow_enrich_lock:
            ...
            for key, value in list(items.items()):
                ...
                if candidate_rank < current_rank and not self._history_rewatch_allowed(value, resume_fields):
                    continue
                if candidate_rank == current_rank and current_rank > 0:
                    candidate_completed = resume_fields.get("history_completed")
                    current_completed = value.get("history_completed")
                    if current_completed is True and candidate_completed is not True:
                        continue
                    if isinstance(current_completed, bool) and candidate_completed is None:
                        resume_fields["history_completed"] = current_completed
                    ...
                    if candidate_position < current_position and candidate_completed is not True:
                        continue
                ...
                history_changed = (
                    ...
                    or item.get("history_completed") != resume_fields.get("history_completed")
                    or self._positive_int(item.get("history_source_updated_at_ms"), 0)
                    != self._positive_int(resume_fields.get("history_source_updated_at_ms"), 0)
                    or self._sanitize_history_playback(item.get("history_playback"))
                    != self._sanitize_history_playback(resume_fields.get("history_playback"))
                ...
                if history_changed:
                    ...
                    if candidate_rank != current_rank and "history_completed" not in resume_fields:
                        item.pop("history_completed", None)
                    item.update(resume_fields)
                    if (
                            "alist_resource_provider" in resume_fields
                            and not resume_fields["alist_resource_provider"]):
                        item.pop("alist_resource_provider", None)
                    previous_playback = self._sanitize_history_playback(value.get("history_playback"))
                    candidate_playback = self._sanitize_history_playback(resume_fields.get("history_playback"))
                    navigation_changed = (
                        self._history_resource_identity(previous_playback)
                        != self._history_resource_identity(candidate_playback)
                        or self._history_drive_identity(previous_playback)
                        != self._history_drive_identity(candidate_playback)
                    )
                    if navigation_changed:
                        item.pop("last_play_route", None)
                    elif resource_id and resource_id != previous_resource_id:
            ...
            if changed:
                self._save_follow_state(items)
            return changed + tombstone_changed
    ...
    def _history_is_complete(history):
        if not isinstance(history, dict):
            return False
        if isinstance(history.get("completed"), bool):
            return history["completed"]
        playback = history.get("history_playback")
        if isinstance(playback, dict) and isinstance(playback.get("completed"), bool):
    ...
    def _history_can_resume(history):
        if not isinstance(history, dict):
            return False
        if isinstance(history.get("completed"), bool) and history.get("completed"):
            return False
        if isinstance(history.get("history_completed"), bool) and history.get("history_completed"):
            return False
        playback = history.get("history_playback")
        if isinstance(playback, dict) and playback.get("completed") is True:
            return False
        try:
            position = int(history.get("position") or history.get("history_position") or 0)
            duration = int(history.get("duration") or history.get("history_duration") or 0)
        except Exception:
            return False
        return 0 < position < duration
    ...
    def _alist_detail_from_metadata(self, raw_id, metadata, deadline=None):
        ...
        try:
            ...
            pass
        try:
            remembered = self._remembered_route_detail(raw_id, item, base_vod)
            if remembered:
    ...
    def _remembered_route_detail(self, raw_id, item, base_vod):
        route = item.get("last_play_route") if isinstance(item.get("last_play_route"), dict) else {}
        play_id = str(route.get("playId") or "").strip()
        resource_id = str(route.get("resourceId") or item.get("alist_vod_id") or "").strip()
        resource_mode = str(
            route.get("resourceMode") or item.get("alist_resource_mode") or "vod"
        ).strip().lower()
        season = self._positive_int(route.get("season"), 0)
        episode = self._positive_int(route.get("episode"), 0)
        backend = str(route.get("backend") or "").strip()
        if backend and backend != self._resource_capability_identity():
            return None
        if (
                not play_id or not resource_id or not season or not episode
                or resource_mode not in self.RESOURCE_SEARCH_MODES
                or not self._resource_id_persistable(resource_id, resource_mode)
                or not re.match(r"^(?:\d+@[^\s?#]+|\d+-\d+|\d+)$", play_id)
                or self._contains_url_reference(self._unquote_limited(play_id))):
            return None
        if resource_mode == "plugin" and self._atvp_plugin_play_context(play_id) is None:
            # The persisted handle is safe, but its raw plugin play ID is memory-only.
            # Fall through to the bound resource detail so the same handle is rebuilt.
            return None
        history_episode = str(item.get("history_episode") or "").strip()
        route_episode = "S%02dE%02d" % (season, episode)
        if history_episode and history_episode != route_episode:
            return None
        name = str(route.get("name") or route_episode).strip()[:256]
        provider = self._resource_provider_key(
            route.get("resourceProvider"), item.get("alist_resource_provider"), resource_id,
        )
        followplay = self._build_followplay(
            play_id, item, resource_id, season, episode, name,
            resource_mode=resource_mode, resource_provider=provider,
        )
        if not followplay:
            return None
        merged = self._merge_resource_vods(
            [{
                "vod_play_from": "上次成功线路",
                "vod_play_url": "%s$%s" % (name, followplay),
                "resource_id": resource_id,
                "_resource_mode": resource_mode,
                "group_seasons": [season],
                "group_providers": [provider],
                "group_quality": [dict(route.get("quality") or {})],
            }],
            item, raw_id, base_vod, preferred_resource_id=resource_id,
        )
        if merged:
            self._schedule_entry_resource_preheat([item])
        return merged
    ...
    def _submit_resource_mode_search(
    ...
    def _resource_layered_output_active(self):
    ...
    def _resource_candidates(
        ...
        if self._resource_search_layered_shadow_enabled:
            try:
                ...
                pass
        ordered = self._resource_output_candidate_order(
            rows,
            item,
            bound=bound,
            cached_rows=cached_rows if supplement_modes else (),
    ...
    def _follow_preheat_current_item(self, fallback):
        fallback = dict(fallback or {})
        items = self._follow_memory.get("items") if isinstance(self._follow_memory, dict) else {}
        if not isinstance(items, dict):
            return fallback
        tmdb_id = self._positive_int(fallback.get("tmdb_id"), 0)
        source_id = str(fallback.get("source_id") or "").strip()
        current = items.get(str(tmdb_id)) if tmdb_id else None
        if not isinstance(current, dict) and source_id:
            current = next((
                value for value in items.values()
                if isinstance(value, dict) and str(value.get("source_id") or "").strip() == source_id
            ), None)
        return dict(current) if isinstance(current, dict) else fallback

    def _follow_route_binding_ready(self, item):
        item = self._follow_preheat_current_item(item)
        route = item.get("last_play_route") if isinstance(item.get("last_play_route"), dict) else {}
        play_id = str(route.get("playId") or "").strip()
        resource_id = str(route.get("resourceId") or item.get("alist_vod_id") or "").strip()
        resource_mode = str(
        ...
        if backend and backend != self._resource_capability_identity():
            return False
        if (
                not play_id or not resource_id or season <= 0 or episode <= 0
                or resource_mode not in self.RESOURCE_SEARCH_MODES
                or not self._resource_id_persistable(resource_id, resource_mode)
                or not re.match(r"^(?:\d+@[^\s?#]+|\d+-\d+|\d+)$", play_id)
                or self._contains_url_reference(self._unquote_limited(play_id))):
            return False
        history_episode = str(item.get("history_episode") or "").strip()
        return not history_episode or history_episode == "S%02dE%02d" % (season, episode)

    def _follow_preheat_snapshot(self):
        items = [
            item for item in (self._follow_memory.get("items") or {}).values()
            if isinstance(item, dict) and str(item.get("title") or "").strip()
        ]
        ready = sum(1 for item in items if self._follow_route_binding_ready(item))
        with self._cache_lock:
            state = getattr(self, "_follow_preheat_state", None)
            state = dict(state) if isinstance(state, dict) else {}
            pending = list(state.get("pending") or [])
            active = dict(state.get("active") or {})
            total = self._positive_int(state.get("total"), 0) or len(items)
            processed = min(total, self._positive_int(state.get("processed"), 0))
            route_jobs = len(self._route_probe_jobs)
            started_at = self._positive_int(state.get("started_at"), 0)
            running = bool(started_at and (pending or active or route_jobs or processed < total))
        return {
            "total": total,
            "processed": processed,
            "ready": min(total, ready),
            "route_jobs": route_jobs,
            "running": running,
            "started_at": started_at,
        }

    def _start_follow_preheat_all(self, items):
        source_items = []
        identities = set()
        for source in items or []:
            ...
            if tmdb_id and not item.get("media_type"):
                item["media_type"] = "tv"
            identity = self._entry_resource_preheat_key(item)
            if not identity or identity in identities:
                continue
            identities.add(identity)
            source_items.append(item)
        with self._cache_lock:
            current = getattr(self, "_follow_preheat_state", None)
            current = current if isinstance(current, dict) else {}
            current_running = bool(
                current.get("generation") == self._cache_generation
                and (current.get("pending") or current.get("active") or self._route_probe_jobs)
            )
            if current_running:
                result = self._follow_preheat_snapshot()
                result["accepted"] = False
                return result
            self._follow_preheat_state = {
                "generation": self._cache_generation,
    ...
    def _drain_follow_preheat_batch(self):
        with self._cache_lock:
            state = getattr(self, "_follow_preheat_state", None)
            if (
                    not isinstance(state, dict)
                    or state.get("generation") != self._cache_generation
                    or state.get("draining")):
                return
            state["draining"] = True
        try:
            while True:
                with self._cache_lock:
                    ...
                    if not pending or len(self._resource_entry_preheat_jobs) >= self.RESOURCE_ENTRY_PREHEAT_LIMIT:
                        break
                    item = dict(pending.pop(0))
                    key = self._entry_resource_preheat_key(item)
                current = self._follow_preheat_current_item(item)
                if self._follow_route_binding_ready(current):
                    self._complete_follow_preheat_resource(current, True)
                    continue
                ready_rows = self._ready_resource_rows(current)
                if ready_rows:
                    self._complete_follow_preheat_resource(current, True)
                    continue
                with self._cache_lock:
                    ...
                    if not isinstance(state, dict) or state.get("generation") != self._cache_generation:
                        return
                    state.setdefault("active", {})[key] = item
                    already_running = key in self._resource_entry_preheat_jobs
                if already_running:
                    continue
                scheduled = self._schedule_entry_resource_preheat([item])
                if scheduled:
                    continue
                ready_rows = self._ready_resource_rows(item)
                self._complete_follow_preheat_resource(item, bool(ready_rows))
        finally:
            with self._cache_lock:
                state = getattr(self, "_follow_preheat_state", None)
                if isinstance(state, dict):
                    ...
                    restart = bool(
                        state.get("pending")
                        and len(self._resource_entry_preheat_jobs) < self.RESOURCE_ENTRY_PREHEAT_LIMIT
                    )
                else:
                    restart = False
            if restart:
    ...
    def _complete_follow_preheat_resource(self, item, resource_ready):
        item = self._follow_preheat_current_item(item)
        key = self._entry_resource_preheat_key(item)
        if resource_ready and not self._follow_route_binding_ready(item):
            try:
                self._warm_follow_item_route(item, self._ready_resource_rows(item))
            except Exception:
                pass
        with self._cache_lock:
            state = getattr(self, "_follow_preheat_state", None)
    ...
    def _warm_follow_item_route(self, item, rows):
        groups = []
        for row in rows or []:
            detail = self._validated_resource_detail(row)
            vod = self._payload_first_vod(detail)
            ...
            rewritten = self._rewrite_resource_vod(
                vod, item, resource_id, mode=mode,
                provider_hint=self._resource_provider_key(
                    row.get("provider"), row.get("type"), row.get("type_name"),
                    row.get("vod_remarks"), row.get("source"), resource_id,
                ),
            ...
            if not rewritten:
                continue
            rewritten["_resource_mode"] = mode
            groups.extend(self._split_resource_vod_groups(rewritten))
        if not groups:
            return False
        raw_id = str(item.get("source_id") or "").strip()
        if not raw_id and self._positive_int(item.get("tmdb_id"), 0):
            raw_id = "tmdb:tv:%s" % self._positive_int(item.get("tmdb_id"), 0)
        self._merge_resource_vods(
            groups,
            item,
            raw_id,
            {"vod_name": str(item.get("title") or "")},
        ...
        return self._follow_route_binding_ready(item)
    ...
    def _schedule_supplement_resource_search(
        ...
        def worker():
            ...
            try:
                ...
                if shadow_payload is not None:
                    legacy_rows, shadow_rows, shadow_modes, candidate_rows = shadow_payload
    ...
    def _validated_playable_detail(
        ...
        for index, group in ranked_groups:
            ...
            for play_index, play_id in enumerate(play_ids):
                ...
                checked = None
                if (self._int_value((output or {}).get("parse"), 0) == 0
                ...
                if self._safe_atvp_play_output(output, deadline=play_deadline):
                    verified = True
                    verified_probe = None
                    verified_output = output
                    verified_play_id = play_id
    ...
    def _rewrite_resource_vod(
        ...
        for group_index, group in enumerate(raw_groups):
            ...
            source_name = self._standardize_resource_name(source_name) or "AList资源"
            if mode:
    ...
    def _merge_resource_vods(self, vods, item, raw_id, base_vod, preferred_resource_id=""):
        ...
        for index, source in enumerate(sources):
            ...
            for part_index, part in enumerate(real_parts):
                ...
                if separator and url:
                    episodes.append({
                        "name": name,
                        "url": url,
                        "selected": not resume_ready and index == 0 and part_index == 0,
                    })
            group_url = real_group
            ...
            flags.append({
                "flag": output_source,
                "urls": group_url,
                "position": -1 if resume_ready or index > 0 else 0,
    ...
    def _schedule_route_quality_save(
    ...
    def _resolve_addresses(self, host, port, deadline=None):
    ...
    def _route_security_policy(self):
        origins = []
        configured = (
    ...
    def _pinned_media_request_blocking(
    ...
    def _safe_atvp_play_output(self, output, deadline=None):
        if not isinstance(output, dict):
            return False
            ...
            if cached_probe is not None:
                self._remember_successful_follow_route(
                    route_payload,
                    {
                        ...
                        "resourceId": resource_id,
                        "resourceProvider": route_payload.get("resourceProvider"),
                        "name": route_payload.get("name") or route_payload.get("episodeName"),
                    },
                    target,
                    cached_probe,
                    expected_generation=generation,
    ...
    def _inject_resume(self, output, parsed):
        ...
        playback = {}
        if history:
            if history.get("_follow_history_conflict"):
                return
    ...
    def _followed_tmdb_detail(self, raw_id):
        ...
        tmdb_match = re.match(r"^tmdb:tv:(\d+)$", raw_id)
        lookup = tmdb_match.group(1) if tmdb_match else raw_id
        item_key, current = self._follow_item_lookup(lookup)
        ...
        if tmdb_id:
            ...
            content_parts.append("TMDB尚未收录，已使用本地连续剧身份；收录后自动迁移且保留播放进度与线路。")
            self._refresh_follow_page_async([item])
        return {"list": [{
            "vod_id": raw_id,
            "vod_name": names,
            "vod_pic": str(item.get("pic") or ""),
    ...
    def _short_error(self, exc, limit=220):
...
# begin vendored module: resource_row_identity.py
import base64
import re
...
def build_resource_row_identity(row_or_id: Any) -> str:
    ...
    if re.match(r"^(?:https?://|magnet:|ed2k:)", decoded, re.I):
        ...
        return "url:" + identity if identity else ""
    return "id:%s:%s" % (mode or "unknown", raw)
# end vendored module: resource_row_identity.py

# begin vendored module: resource_candidate_merge.py
    ...
    *,
    ...
    for value in rows or []:
        ...
        if identity in positions:
            ...
            merged.append(row)
    return merged
# end vendored module: resource_candidate_merge.py

# begin vendored module: resource_candidate_ordering.py
...
# end vendored module: resource_candidate_ordering.py

# begin vendored module: resource_candidate_pipeline.py
...
# end vendored module: resource_candidate_pipeline.py

# begin vendored module: resource_candidate_shadow.py
from typing import Any, Callable, Iterable
...
# end vendored module: resource_candidate_shadow.py

# begin vendored module: resource_candidate_shadow_policy.py
import hashlib
...
# end vendored module: resource_candidate_shadow_policy.py

# begin vendored module: resource_candidate_shadow_composition.py
from typing import Any, Callable, Iterable
...
def compose_resource_candidate_shadow(
    ...
    *,
    ...
    return result
# end vendored module: resource_candidate_shadow_composition.py

# begin vendored module: resource_candidate_shadow_background.py
import hashlib
from typing import Any
...
# end vendored module: resource_candidate_shadow_background.py

# begin vendored module: resource_candidate_shadow_runtime.py
...
def run_background_resource_candidate_shadow(
    ...
    *,
    ...
    with owner._resource_candidate_shadow_lock:
        ...
        return result
# end vendored module: resource_candidate_shadow_runtime.py

# begin vendored module: resource_models.py
...
# end vendored module: resource_models.py

# begin vendored module: resource_schema.py
from dataclasses import dataclass
...
def supplement_streams(payload: Any):
    ...
    return streams
# end vendored module: resource_schema.py

# begin vendored module: resource_shadow.py
import base64
import json
import re
import unicodedata
...
# end vendored module: resource_shadow.py

# begin vendored module: resource_provider.py
from dataclasses import dataclass
...
# end vendored module: resource_provider.py

# begin vendored module: resource_search_plan.py
from dataclasses import dataclass
from typing import Any, Iterable, Tuple
...
def build_resource_search_plan(
    ...
    return tuple(steps)
# end vendored module: resource_search_plan.py

# begin vendored module: resource_search_shadow.py
from dataclasses import dataclass
...
# end vendored module: resource_search_shadow.py

# begin vendored module: resource_search_v70_adapter.py
from dataclasses import dataclass, field
...
def build_v70_layered_resource_shadow(
    ...
    return tuple(batches)
# end vendored module: resource_search_v70_adapter.py

# begin vendored module: resource_search_shadow_runtime.py
...
def run_resource_search_layered_shadow(
    ...
    with owner._resource_search_layered_shadow_lock:
        ...
        return result
# end vendored module: resource_search_shadow_runtime.py
import hashlib
import threading
import time
...
PLAYBACK_SYNC_CONTRACT = "alist-tvbox-playback-sync-1.51.1"
...
def _v80_history_event(row, owner=None):
    ...
    if isinstance(row.get("completed"), bool):
        event["completed"] = row["completed"]
    for key, aliases in (
        ("clientKey", ("clientKey", "client_key")),
        ("sourceSubgroupName", ("sourceSubgroupName", "source_subgroup_name")),
        ("driveDirId", ("driveDirId", "drive_dir_id")),
        ("driveShareKey", ("driveShareKey", "drive_share_key")),
        ("drivePath", ("drivePath", "drive_path")),
    ):
        value = _history_clip_text(_v80_history_first(row, *aliases), 4096).strip()
        if value:
            event[key] = value
    for key, aliases in (
        ("playlistIndex", ("playlistIndex", "playlist_index")),
        ("sourceGroupIndex", ("sourceGroupIndex", "source_group_index")),
        ("sourceIndex", ("sourceIndex", "source_index")),
        ("sourceSubgroupIndex", ("sourceSubgroupIndex", "source_subgroup_index")),
    ):
        value = _v80_history_first(row, *aliases)
        if value is not None and not isinstance(value, bool):
            event[key] = max(0, _v80_history_int(value, 0))
    return event
...
def _v80_history_rank(row):
    return (
        ...
        1 if row.get("completed") is True else 0,
...
def v80_history_active_cid(owner):
    ...
    try:
        config = jclass("com.fongmi.android.tv.bean.Config").vod()
        cid = _v80_history_int(config.getId() if config is not None else 0, 0)
        return max(0, cid)
    except Exception:
        return 0
...
def _v80_history_login_unbounded(
    ...
    uid = _v80_history_int(
        value.get("userId") or value.get("id"), 0
...
def _v80_history_state_get(owner):
    ...
    if state.get("version") not in (1, 2) or not isinstance(records, list):
        raise RuntimeError("AList-TVBox 播放记录同步状态损坏")
...
def _v80_history_fetch_locked(owner, legacy_fetch, stateful=False):
    ...
    owner._v80_history_tombstones = list(deleted)
    ...
    owner._v80_history_playback_states = {
        (event["sourceKind"], event["sourceKey"], event["vodId"]): dict(event)
