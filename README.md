...
2026-08-18 的项目级安全复核关闭了 P4 响应边界的两个灰度阻断项：Douban JSON/HTML、TMDB 与想看动作的固定请求 owner 全部显式关闭 Requests 自动重定向；`_resolve_user_id()` 对 3xx 只解析同源 `Location`，不跟随也不读取响应体，对 `200` 使用既有 `256 KiB` 上限、总 deadline 和单一关闭 owner。三个 P4 overlay 聚焦文件 `66 passed`，未新增通用 session、redirect、retry、cache 或 executor 框架。该批次候选为 `862377` bytes、SHA256 `C1ACAB802121E3F69ADEA0EBF1AB271C14015124AA28D2D1F8F58F97C8481B7D`；开发源码 README 已为 `10` 个 P1 chunk 和 `36` 个根目录 Python module 建立 `46/46` 唯一中文维护别名，聚焦校验 `1 passed`。该批次内容寻址证据固定写入 `work/v80-p5-5f-redirect-boundary-alias-closure-r6-20260818.json`，公开 V70 与根索引继续保持冻结。

随后进行的原子 owner 审计确认，未探测播放 URL 仍可能绕过严格地址策略，且 P2 分层引擎仍是 shadow-only。最小修复只扩展 V80 route overlay：`_safe_atvp_play_output`、前台播放两处 fallback、资源详情探测前检查与 fallback 全部复用严格 DNS/地址策略和既有 deadline；三个共享分片恢复为冻结基线。当前候选为 `862581` bytes、SHA256 `87DCAC75E7F60CA70219EA99C238940E756D53D17A82D2FE684622A38CD5BADC`，build write/check 通过，定向组合闭包为 `318 passed`。原子证据为 `work/v80-source-owner-audit-20260818.json`。下一步不是直接灰度，而是先增加 raw-row-preserving 分层合并器、私有 V80 controlled switch 和独立 `id/version/index/staging`；完成文档与静默双指纹后，再从受信 r4 按显式 DAG 续跑必要门禁。

P2 raw-row-preserving 组合器与 private-V80-only controlled switch 现已完成本地接管验证。前台和背景共用 `_resource_output_candidate_order`，switch 默认关闭，只在 raw-plugin 私有配置中启用；combiner 异常只执行一次 legacy fallback，不增加 retry 或新框架。当前开发候选为 `870797` bytes、SHA256 `0CEBC73A78BCC8C7853A6BD0F0C78F4D95DD786C861425F9E0A4EC40FA0583F9`；Macro A 为 `50000` 例、`14736` 个受控差异、`0 errors`，Macro B 为 `50000 equal / 0 different / 0 errors`，Chaos `13/13`，ATVP、双运行时和分类合同通过。独立私有 staging 已生成在 `private/v80/`：`id=douban_tmdb_follow_single_v80_private`、`version=80`，staged source 为 `870801 / 049C722515F6851C379969C2886FA466EDD9FC9478B6B6F591E757DEEEDDCB97`；私有索引使用相对 `staging/` 路径，公开 V70 与根 `spiders_v2.json` 未变。controlled-switch 与私有发布报告分别为 `work/v80-p2-controlled-output-switch-evidence-20260818.json` 和 `work/v80-private-release-evidence-20260818.json`。服务器、MuMu 和真实 FongMi 尚未操作；下一步是静默双指纹与变化节点 DAG closure。
