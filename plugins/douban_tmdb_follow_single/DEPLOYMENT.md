...
本插件用于豆瓣/TMDB 浏览、剧集追更、AList-TVBox 资源搜索、线路评分和 History 续播。当前发布版本为 v48，适配 AList-TVBox 1.42.0 raw Python 插件，由 AList-TVBox 生成订阅后交给 FongMi/TvBox 使用。
...
5. 在详情中先选季集，再选择评分较高或已有续播记录的线路。

添加、取消、标记已看和同步结果都会显示持久状态卡，不需要依赖 Toast 判断操作是否成功。
...
- 用户名或密码留空：保持只读，不影响 History 匹配、续播和过滤器。
