...
本插件用于豆瓣/TMDB 浏览、剧集追更、AList-TVBox 资源搜索、线路评分和 History 续播。当前发布版本为 v49，适配 AList-TVBox 1.42.0 raw Python 插件，由 AList-TVBox 生成订阅后交给 FongMi/TvBox 使用。

不要把 `.py` 文件直接添加为普通 FongMi 站点，也不要手工填写外层 `api`、`token`、`secret`、`loader`、`source` 或 `raw`。
...
1. 进入豆瓣或 TMDB 剧集分类并打开详情。
2. 切换到“加入追更”后点击剧集卡。
3. 在“追更动态”查看新集，在“追更管理”查看全部记录。
4. 标记已看或取消追更时，先进入对应管理模式，再点击项目和确认卡。
5. 需要查看同步状态、立即重试或检测通讯时，打开“追更管理”；首页不再提供独立“云端历史”分类。
6. 在详情中先选季集，再选择评分较高或已有续播记录的线路。
...
追更动作只提交一次追更页定向刷新。直接刷新失败时最多等待约 1 秒再检查一次当前可见追更页，普通浏览分类不会被后台补全反复刷新。
...
- “追更管理”始终提供“立即同步 History”“检测通讯”和“本地收藏自动追更”，即使还没有追更剧集也可使用。
